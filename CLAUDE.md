...
- **Game loop**: `loop(ts)` is `requestAnimationFrame`-driven; accumulates `dt` into `dropAccum` and steps the piece down once `dropAccum >= dropInterval`. Pausing cancels the frame (`cancelAnimationFrame(animId)`) and resuming restarts the loop — pause is not a flag checked inside the loop.
